  ...
  Query: {
    ...
    // },
     getAllUsers: async (parent: any, context: any, req: Request, args: any) => {
      ...
      const perPage = 5;
      let after = null; // Initialize cursor
    
      if (page > 1) {
        // Fetch the cursor for the previous page
        const prevPage = page - 1;
        const prevResult = await User.find()
          .skip((prevPage - 1) * perPage)
          .limit(perPage)
          .exec();
      ...
      const result = await User.find({
        cursor: {
          $gt: after, // Filter items after the specified cursor
        },
      })
        .limit(perPage)
        .exec();
      ...
      // Process the result and return edges
      const edges = result.map((user) => ({
      ...
      return {
        edges,
        pageInfo: {
          hasNextPage: true, // Indicates if there are more pages
        },
      };
    }
  },
